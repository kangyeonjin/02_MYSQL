-- view
-- selece 쿼리문을 저장한객체 >가상테이블이라고 불린다
-- 실질적인 데이터를 물리적으로 저장하고 있지 않음
-- 테이블을 사용하는것과같이 동일하게 사용가능하다
...
create view hansik as
    select * from tbl_menu -- 베이스테이블
             where category_code =4;

select * from hansik;

-- 베이스테이블(tbl_menu) 정보가 변경되면 view의 결과도 같이 변경된다
insert into tbl_menu values(null, '식혜맛국밥',5500,4,'Y');
select  * from hansik;

-- view를 통해 데이터를 넣기
-- 1. view통한 insert(view auto_increment가 없으므로, pk컬럼의 값을 지정해줘야한다)
...
-- 1.뷰정의에 포함되지 않은 컬럼을 조작하는경우
-- 2.뷰에 포함되지 않는컬럼중에 베이스가 되는 테이블 컬럼이 NOT NULL제약조건에 지정된경우
-- 3. 산술표현식이 정의된경우
-- 4. JOIN을 이용해 여러 테이블을 연결한경우
-- 5. DISTINCT를 포함한 경우
-- 6.그룹함수나 GROUPBY를 포함한 경우

-- VIEW삭제
DROP VIEW hansik;

-- view에 쓰인 subquery 연산 결과도 컬럼으로 사용가능하다
create view hansik as
